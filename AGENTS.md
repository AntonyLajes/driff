...
**Compare windows (product intent):** The push webhook still supplies `beforeSha` / `afterSha` to `process_release` today, which can miss work when only a bump commit appears on the push. Target behavior — **summarize everything during the previous build** on build bumps, and **everything during the previous marketing version** on major bumps — is specified in `docs/release-compare-windows.md` (anchors derived from prior `releases.head_sha` and optional era fields). Implementation should converge on that doc.
