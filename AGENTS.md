...
**Driff Lab (V1 P0, validation contract):** deterministic GitHub scenarios live in `src/lab/fixtures/` and are validated by `src/lab/scenario.ts`. Run `npm run lab:validate -- <scenario.json>` before replay work. A scenario uses schema version `1`, has one repository, ordered event offsets, unique delivery IDs, raw GitHub payloads, and expected job types. Payload repository names must match the scenario repository. The validator has no network or database side effects. Future replay must use the real signed webhook boundary, allow localhost by default, and require an explicit development-host allowlist for remote targets; never add a production bypass route for lab data.
