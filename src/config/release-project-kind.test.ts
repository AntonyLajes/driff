...
describe("config/release-project-kind", () => {
  it("should parse normalized kind strings", () => {
    ...
    expect(parseReleaseProjectKind("react_native_expo")).toBe(
      "react_native_expo",
    );
  ...
  it("should classify supported kinds", () => {
    ...
    expect(isSupportedReleaseProjectKind("android_gradle")).toBe(true);
    expect(isSupportedReleaseProjectKind("python_pyproject")).toBe(true);
  });
  ...
  it("should map ios_pbx to pbx with empty plist sentinel", () => {
    expect(
      applyReleaseKindAndFilePath("ios_pbx", "x.xcodeproj/project.pbxproj"),
    ).toEqual({
  ...
  it("should map react_native_expo to expo path", () => {
    expect(
      applyReleaseKindAndFilePath("react_native_expo", "app.config.ts"),
    ).toEqual({
      releaseInfoPlistPath: "",
      releaseProjectPbxprojPath: null,
  ...
  it("should keep generic version files as unified-only sources", () => {
    expect(
      applyReleaseKindAndFilePath("android_gradle", "app/build.gradle"),
    ).toEqual({
  ...
  it("should infer kind from legacy with expo precedence", () => {
    ...
    expect(
      inferKindAndPathFromLegacyPaths("ios/Info.plist", "x.pbxproj", null),
    ).toEqual({
      kind: "ios_pbx",
      path: "x.pbxproj",
    });
    expect(
      inferKindAndPathFromLegacyPaths("ios/Info.plist", null, null),
    ).toEqual({
      kind: "ios_plist",
      path: "ios/Info.plist",
    });
